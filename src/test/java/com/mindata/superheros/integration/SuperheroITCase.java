...
public class SuperheroITCase extends IntegrationITCase{
    ...
    public void getSuperhero() {
        ...
        assertThat(responseEntity.getStatusCode(), is(OK));
        assertThat(responseEntity.getHeaders().getContentType(), is(APPLICATION_JSON));
        assertThat(requireNonNull(responseEntity.getBody()).getName(), equalTo("Superman"));
    }
    ...
    public void getSuperherosFilterByOriginSubString() {
        String URL = SUPERHERO_URL + "?origin=New York";
        ResponseEntity<SuperheroResponse[]> responseEntity = restTemplate.getForEntity(URL, SuperheroResponse[].class);
    ...
    public void postSuperhero() {
        SuperheroRequest superheroRequest = new SuperheroRequest("Spiderman", "Male", "New York, US, Earth", Date.valueOf("1962-08-10"));
        ...
        assertThat(responseEntity.getHeaders().getContentType(), is(APPLICATION_JSON));
        assertThat(requireNonNull(responseEntity.getBody()).getName(), equalTo("Spiderman"));
        assertThat(requireNonNull(responseEntity.getBody()).getId(), equalTo("3"));
    }

    @Test
    public void updateSuperhero() {
        SuperheroRequest superheroRequest = new SuperheroRequest("Superman", "Male", "Buenos Aires, Argentina, Earth", Date.valueOf("1984-07-04"));
        HttpEntity<SuperheroRequest> requestSuperheroEntity = new HttpEntity<>(superheroRequest);
        ...
        assertThat(responseEntity.getHeaders().getContentType(), is(APPLICATION_JSON));
        assertThat(requireNonNull(responseEntity.getBody()).getName(), equalTo("Superman"));
        assertThat(requireNonNull(responseEntity.getBody()).getOrigin(), equalTo("Buenos Aires, Argentina, Earth"));
        ...
        assertThat(requireNonNull(updatedResponseEntity.getBody()).getName(), equalTo("Superman"));
        assertThat(requireNonNull(updatedResponseEntity.getBody()).getOrigin(), equalTo("Buenos Aires, Argentina, Earth"));
    }
}
